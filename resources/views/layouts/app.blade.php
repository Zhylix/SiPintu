        ...
        <div @click.away="mobileMenuOpen = false" class="w-72 bg-white h-full shadow-2xl flex flex-col justify-between relative z-10">
            ...
            <nav class="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto">
                @if(auth()->user()->isAdmin())
                    ...
                    <a href="{{ route('admin.jurusan.index') }}" class="flex items-center px-3 py-2 text-xs font-bold rounded-xl transition-all {{ request()->routeIs('admin.jurusan.*') ? 'bg-emerald-700 text-white shadow-md shadow-emerald-700/20' : 'text-slate-700 hover:text-emerald-800 hover:bg-emerald-50' }}">
                ...
                @elseif(auth()->user()->isStudent() || auth()->user()->isAlumni())
                    <div class="px-3 pb-2 text-[10px] font-extrabold text-emerald-900 uppercase tracking-wider">{{ auth()->user()->isAlumni() ? 'Portal Alumni SMKN 1 Bangsri' : 'Portal Siswa SMKN 1 Bangsri' }}</div>
                    ...
                    <a href="{{ route('student.dashboard') }}" class="flex items-center px-3 py-2 text-xs font-bold rounded-xl transition-all {{ request()->routeIs('student.dashboard') ? 'bg-emerald-700 text-white shadow-md shadow-emerald-700/20' : 'text-slate-700 hover:text-emerald-800 hover:bg-emerald-50' }}">
                        <svg class="w-4 h-4 mr-2.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                        {{ auth()->user()->isAlumni() ? 'Dashboard Alumni' : 'Dashboard Siswa' }}
                    </a>
            ...
            <aside class="hidden md:flex md:w-64 bg-white border-r border-slate-200 flex-col shrink-0">
                ...
                <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
                    @if(auth()->user()->isAdmin())
                        ...
                        <a href="{{ route('admin.jurusan.index') }}" class="flex items-center px-3 py-2.5 text-sm font-bold rounded-xl transition-all {{ request()->routeIs('admin.jurusan.*') ? 'bg-emerald-700 text-white shadow-md shadow-emerald-700/20' : 'text-slate-700 hover:text-emerald-800 hover:bg-emerald-50' }}">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path></svg>
                            Alumni & Jurusan
                        </a>
...
;                        <a href="{{ route('admin.analytics.index') }}" class="flex items-center px-3 py-2.5 text-sm font-bold rounded-xl transition-all {{ request()->routeIs('admin.analytics.*') ? 'bg-emerald-700 text-white shadow-md shadow-emerald-700/20' : 'text-slate-700 hover:text-emerald-800 hover:bg-emerald-50' }}">
                    ...
                    @elseif(auth()->user()->isStudent() || auth()->user()->isAlumni())
                        <div class="px-3 pb-2 text-[11px] font-extrabold text-emerald-900 uppercase tracking-wider">{{ auth()->user()->isAlumni() ? 'Portal Alumni SMKN 1 Bangsri' : 'Portal Siswa SMKN 1 Bangsri' }}</div>
