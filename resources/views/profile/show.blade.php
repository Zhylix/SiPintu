...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- User Card Summary -->
    <div class="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="flex items-center space-x-5">
            <div class="w-16 h-16 rounded-2xl bg-emerald-700 text-white flex items-center justify-center font-black text-2xl shadow-md">
                {{ $user->initials() }}
            </div>
            <div>
                <h2 class="text-xl font-black text-emerald-950">{{ $user->name }}</h2>
                <div class="flex flex-wrap items-center gap-2 mt-1.5">
                    <span class="px-2.5 py-0.5 rounded-full text-xs font-extrabold uppercase bg-emerald-100 text-emerald-800 border border-emerald-300">
                        Peran: {{ $user->getUserTypeName() }} ({{ $user->role }})
                    </span>
                    @if($user->external_id)
                        <span class="text-xs text-slate-600 font-medium">External ID: <code class="text-emerald-800 font-mono font-bold">{{ $user->external_id }}</code></span>
                    @endif
                </div>
            </div>
        </div>

        <div class="text-right text-xs text-slate-600 font-semibold">
            <div>Terdaftar sejak: <span class="text-slate-900 font-bold">{{ $user->created_at?->format('d M Y') }}</span></div>
            <div class="mt-1">Status Akun: <span class="text-emerald-700 font-black uppercase">{{ $user->status }}</span></div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
            <h3 class="text-base font-black text-emerald-950 flex items-center space-x-2 pb-3 border-b border-slate-100">
        ...
        <div class="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
            ...
            @if($user->isStudent())
                <div class="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-xs text-emerald-900 font-medium">
                    <p class="font-black text-emerald-950 mb-1">Akun Siswa Tersinkronisasi SIJUNA</p>
                    Kata sandi untuk akun Siswa bersumber dari SIJUNA API dan tidak dapat diubah dari Gateway.
                </div>
            @else
                <form method="POST" action="{{ route('profile.password') }}" class="space-y-4 pt-1">
                    @csrf
                    @method('PUT')
