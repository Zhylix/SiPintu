    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5">
        ...
        <div>
            ...
            <select name="user_type" required class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white text-sm focus:border-indigo-500 focus:outline-none">
                <option value="student" {{ old('user_type', $user->user_type) === 'student' ? 'selected' : '' }}>Siswa (Role: student)</option>
                <option value="teacher" {{ old('user_type', $user->user_type) === 'teacher' ? 'selected' : '' }}>Guru / Pendidik (Role: teacher)</option>
                <option value="dudi" {{ old('user_type', $user->user_type) === 'dudi' ? 'selected' : '' }}>DUDI / Mitra Industri (Role: dudi)</option>
                <option value="admin" {{ old('user_type', $user->user_type) === 'admin' ? 'selected' : '' }}>Administrator Gateway (Role: admin)</option>
            </select>
        </div>
